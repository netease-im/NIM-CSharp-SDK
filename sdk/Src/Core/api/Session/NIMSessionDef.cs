    ...
    public class SessionInfo : NimUtility.NimJsonObject<SessionInfo>
    {
        ...
        public void ParseAttachmentInfo()
        {
            if (MsgType != NIMMessageType.kNIMMessageTypeUnknown && !string.IsNullOrEmpty(Attach))
            {
                ...
                {
                    NIMTeamEventData eventData=NIMTeamEventData.Deserialize(Attach);
                    NIMTeamEventDataObject eventDataObject = NIMTeamEventDataObject.Deserialize(Attach);
                    if(eventData!=null&&eventDataObject!=null)
                    {
                        eventData.TeamEvent = eventDataObject.TeamEvent;
                    }
                    Attachment = eventData;
                }
                else
